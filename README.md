...
> **介绍**: EdgeGate 是一个基于嵌入式 Linux(C++17)的工业数据采集网关——从零实现 **Modbus RTU 协议栈** 与 **MQTT 3.1.1 客户端**,通过 RS485/串口轮询现场设备,在边缘完成滤波与迟滞告警后上云,内置 **PTY 虚拟串口 + 软件从站**(寄存器堆可读写、支持故障注入)用于链路自测与演示,支持 CMake 交叉编译到 ARM Cortex-A7 与 x86 双平台。
